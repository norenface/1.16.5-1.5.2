...
public class ComputerBlock extends net.minecraft.block.BlockContainer {
    ...
    public ComputerBlock(int id, Material material) {
        ...
        // 🌟 1.5.2で推奨される基本設定を追加
        this.setHardness(2.0F);          // 硬さ（石と同じくらい）
        this.setResistance(5.0F);        // 爆発耐性
        this.setStepSound(Block.soundMetalFootstep); // 歩いた時の音（金属音）
        this.setUnlocalizedName("pc_block"); // 内部的な名前（言語ファイル用）
  ...
  //  public boolean isItemStackInvalidForSlot(int slot, ItemStack stack) {
   ...
   // }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
        ...
        // --- ここから下はサーバー側（!world.isRemote）のみが実行する ---
        ...
        int currentSlotIndex = player.inventory.currentItem;
        ...
        if (entity.getClickCount() == 0 || timeDiff > 10) {
            ...
            if (currentCount == 2) {
                ...
                entity.resetClickState();
                world.playSoundAtEntity(player, "random.chestclosed", 0.5F, 0.8F);
            }
        }
    ...
    private void recordAndTransfer(ComputerBlockEntity entity, EntityPlayer player, int amount) {
        ...
        int moved = (player.getCurrentEquippedItem() == null) ? before : before - player.getCurrentEquippedItem().stackSize;
    ...
    private void transferInventoryExceptHotbar(ComputerBlockEntity entity, EntityPlayer player) {
        if (player.worldObj.isRemote) return; // サーバー側でのみ実行
        ...
        if (changed) {
            ...
            if (player instanceof net.minecraft.entity.player.EntityPlayerMP) {
                ((net.minecraft.entity.player.EntityPlayerMP)player).sendContainerToPlayer(player.openContainer); }
        }
    }
    ...
    private void undoTransfer(ComputerBlockEntity entity, EntityPlayer player) {
        ...
        if (lastStack != null && count > 0) {
            ...
            if (removed) {
                // プレイヤーの現在の持ち手を確認
                ItemStack currentHeld = player.inventory.getCurrentItem();
                ...
                if (currentHeld == null) {
                    ...
                    if (!player.inventory.addItemStackToInventory(returnStack)) {
                        ...
                        player.dropPlayerItem(returnStack);
                    }
                }
            } else {
                // 1.5.2 には派手な通知メッセージがないので、サーバーログかチャットに送る
                if (!player.worldObj.isRemote) {
                    player.addChatMessage("Storage Error: Failed to remove item");
                }
            }
        }
